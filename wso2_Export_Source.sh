...
export WSO2_CLIENT_ID=8wRI_oF0qhRlkRiGWEVxYDWc6L8a
export WSO2_CLIENT_SECRET=a00IcXNtor9uaVDvLcNgH8nf210a


export WSO2_ADMIN_USERNAME=admin
export WSO2_ADMIN_PASSWORD=Thailife@2019
...
export WSO2_BASE_URL_TOKEN=https://10.102.60.112:8243
export WSO2_BASE_URL_API=https://10.102.60.112:9443
...
get_all_apis() {
	result=$(curl -s -k -H "Authorization: Bearer $(admin_token)" -X GET ${WSO2_BASE_URL_API}/api/am/publisher/v0.11/apis?limit=1000)
	
    
	echo "$result" | jq -rc '.list[]' || { echo "$result"; error "Error while get all apis"; }
	#echo "curl -s -k -H "Authorization: Bearer $(admin_token)" -X GET ${WSO2_BASE_URL_API}/api/am/publisher/v0.11/apis?limit=1000"
}
...
get_api_detail() {
	...
	#echo "	curl -s -k -H "Authorization: Bearer $(admin_token)" ${WSO2_BASE_URL_API}/api/am/publisher/v0.11/apis/${api_id} | jq ."
	curl -s -k -H "Authorization: Bearer $(admin_token)" ${WSO2_BASE_URL_API}/api/am/publisher/v0.11/apis/${api_id} | jq .
}
...
get_all_applications() {
	result=$(curl -s -k -H "Authorization: Bearer $(admin_token)" -H "Content-Type: application/json" -X GET -d "$data" ${WSO2_BASE_URL_API}/api/am/store/v0.11/applications)
...
")
		...
		if [ -z "$id" ]; then
			echo ""
			echo "CREATE: $name ..."
			result=$(curl -s -k -H "Authorization: Bearer $(admin_token)" -H "Content-Type: application/json" -X POST -d "$api_detail" ${WSO2_BASE_URL_API}/api/am/publisher/v0.11/apis/${id})
			...
			echo ""
			echo "UPDATE: $name ..."
			result=$(curl -s -k -H "Authorization: Bearer $(admin_token)" -H "Content-Type: application/json" -X PUT -d "$api_detail" ${WSO2_BASE_URL_API}/api/am/publisher/v0.11/apis/${id})
...
change_status_api() {
	...
	curl -s -k -H "Authorization: Bearer $(admin_token)" -H "Content-Type: application/json" -X POST "${WSO2_BASE_URL_API}/api/am/publisher/v0.11/apis/change-lifecycle?apiId=$api_id&action=$status"
}
...
subscribe_api() {
	...
	curl -s -k -H "Authorization: Bearer $(admin_token)" -H "Content-Type: application/json" -X POST -d "$data" ${WSO2_BASE_URL_API}/api/am/store/v0.11/subscriptions
}
...
unsubscribe_api() {
	...
	do
		...
		curl -s -k -H "Authorization: Bearer $(admin_token)" -X DELETE "${WSO2_BASE_URL_API}/api/am/store/v0.11/subscriptions/${sub_id}"
	done	
}
...
remove_api() {
	...
	curl -s -k -H "Authorization: Bearer $(admin_token)" -X DELETE "${WSO2_BASE_URL_API}/api/am/publisher/v0.11/apis/${api_id}"
}
...
get_all_api_subscription() {
	...
	curl -s -k -H "Authorization: Bearer $(admin_token)" -H "Content-Type: application/json" "${WSO2_BASE_URL_API}/api/am/store/v0.11/subscriptions?apiId=${api_id}"
}

get_application_id_by_name() {
	application_name=$1

	curl -s -k -H "Authorization: Bearer $(admin_token)" -H "Content-Type: application/json" -X GET -d "$data" ${WSO2_BASE_URL_API}/api/am/store/v0.11/applications | jq ".list[] | select(.name == \"${application_name}\") | .applicationId" -r
}
...
get_subscribed_applications() {
	...
	result=$(curl -s -k -H "Authorization: Bearer $(admin_token)" -H "Content-Type: application/json" -X GET "${WSO2_BASE_URL_API}/api/am/store/v0.11/subscriptions?apiId=${api_id}")
